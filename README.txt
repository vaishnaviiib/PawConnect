...
Each line: relative path - description
...
jest.config.unit.mjs - Jest config for unit tests only (no MongoDB setup).
jest.config.integration.mjs - Jest config for integration tests (MongoDB setup).
...
--- tests ---
tests/integration/README.txt - How to run integration tests and .env.test prerequisites.
tests/integration/setup.js - Jest setup for integration suite: .env.test, Mongo connect, clear between tests, drop DB after.
tests/integration/auth.test.js - Jest/Supertest tests for authentication API routes.
tests/integration/dogs.test.js - Jest/Supertest tests for dog API routes.
tests/integration/applications.test.js - Jest/Supertest tests for application API routes.
tests/unit/README.txt - How to run unit tests (no MongoDB required).
tests/unit/pawApi.test.js - Jest tests for client pawApi module with mocked fetch and localStorage.
...
Prerequisites:
  ...
  - MongoDB reachable at the URI you put in .env (for API and for integration tests only)
...
3) Optional quality commands (not required to run the app)
   - npm run lint              (ESLint)
   - npm run test:unit         (Jest unit tests; no MongoDB; see tests/unit/README.txt)
   - npm run test:integration  (Jest integration tests; requires .env.test and MongoDB; see tests/integration/README.txt)
   - npm test                  (runs unit then integration; see Section 7)
...
--- A) Local development (recommended): two terminals, same project root ---
...
Variable: MONGO_URI or MONGO_URL
  ...
  Required: Yes for integration tests; tests/integration/setup.js throws if missing.
...
Variable: MONGO_DB_NAME (optional)
  Meaning:  Database name; default in tests/integration/setup.js is pawlink_test.

Warning: tests/integration/setup.js deletes all documents after each test and calls
...
7c) npm scripts (no extra arguments required by these projects)
  ...
  npm run lint              - eslint .
  npm run test:unit         - jest with jest.config.unit.mjs (no MongoDB)
  npm run test:integration  - jest with jest.config.integration.mjs (requires .env.test)
